...
def apply_kernel_all_channels(img_arr, kernel, channel_first = True):
    ...
    for i in range(img_arr.shape[0] if channel_first else img_arr.shape[-1]):
        ...
        if i != 0:
            result = np.concatenate((result, np.expand_dims(apply_kernel(channel_arr, kernel), axis=channel_dim)), axis = channel_dim)
        else:
...
def is_point_a_question(x, y, all_points):
    ...
    for i in range(4):
        ...
        if not pt_found:
            ...
            pre_pt += y_width
    return ((total_pt_fount >= question_error_threshold_pt) and ((total_width_error/total_pt_fount) <= question_error_threshold_width_avg)), \
            option_points
...
def shift_options(options, count):
    ...
    for i in range(count):
        x = 2*new_options[0][0] - new_options[1][0]
        y = 2*new_options[0][1] - new_options[1][1]
...
def check_if_custom_answer(q_marker, img_arr):
    img_patch = img_arr[q_marker[0][0]: q_marker[0][0]+option_box_height, q_marker[0][1] - custom_text_dist_option: q_marker[0][1] + custom_text_width - custom_text_dist_option]
...
def putting_it_all_together(img_arr, gd_truths, st_answers):
    ...
    if len(custom_ans_idxes) == len(st_answers):
        if (np.array(sorted(custom_ans_idxes)) == np.array(sorted(st_answers))).sum() == len(custom_ans_idxes):
            print("All custom answers found.")
        else:
            print("Did not find all custom answer")
    else:
        print("Did not find all custom answer")
    ...
    for i, act_val in enumerate(act_vals):
        if i in st_answers:
            if int(act_val) == 535500:
                print(f"max idx: {i}")
            st_act_vals.append(act_val)
        else:
            no_st_act_vals.append(act_val)

    print(f"min no st: {min(no_st_act_vals)}")
    print(f"max no st: {max(no_st_act_vals)}")
    print(f"min st: {min(st_act_vals) if  len(st_act_vals) > 0 else 0}")
    print(f"max st: {max(st_act_vals) if  len(st_act_vals) > 0 else 0}")

    if (np.array(gd_truths) == np.array(all_sel_vals)).sum() == 425:
        print("Parsed all correctly.")
    else:
        print("Could not parse all correctly.")

    sel_min_max, un_sel_min_max, sel_all_min, sel_all_max, unsel_all_min, unsel_all_max, sel_idxes, un_sel_idxes = perform_selected_option_analytics(all_kernel_vals, gd_truths, q_markers)
    print(sel_min_max)
    print(un_sel_min_max)
    print(sel_all_min)
    print(sel_all_max)
    print(unsel_all_min)
    print(unsel_all_max)
...
if __name__ == '__main__':
    ...
    img = Image.open("test-images/b-13.jpg")
    if img.mode == "RGBA":
        img_arr = np.asarray(img)[..., :3]
    else:
        img_arr = np.asarray(img)

    gd_truth, st_answers = read_ground_truths("test-images/b-13_groundtruth.txt", st_answer=True)
    putting_it_all_together(img_arr, gd_truth, st_answers)

    print("DONE.")



    # g_kern = create_gaussian_kernel(100, 15)
    # x_vals = np.sum(g_kern, axis=1)
    # plt.plot(x_vals)
    # plt.show()

    # img_arr = np.asarray(Image.open("test-images/c-18.jpg"))
    img = Image.open("test-images/b-13.jpg")
    if img.mode == "RGBA":
        img_arr = np.asarray(img)[..., :3]
    else:
    ...
    # kernel = detail_kernel(13, 2)
    # kernel = create_gaussian_kernel(13, 2)
    # kernel = sharpen_kernel(13, 2)
    ...
    det_img = np.zeros(det_img.shape)

    for x,y in zip(x_vals, y_vals):
        det_img[x-3:x+3,y-3:y+3] = 1
    ...
    for x in q_beg:
        for y in q_beg[x]:
            all_pts = q_beg[x][y]
            for pt in all_pts:
                det_img[pt[0]-6:pt[0]+6, pt[1]-6:pt[1]+6] = 1
    plt.imshow(det_img / 255, cmap="gray")
    plt.show()


    all_vertical_groups = group_questions(q_beg)
    all_vertical_groups = combine_groups(all_vertical_groups)
    for group in all_vertical_groups:
        det_img = np.zeros(det_img.shape)
        for option_list in group["all_options"]:
            for option in option_list:
                det_img[option[0]-5:option[0]+5, option[1]-5:option[1]+5] = 1
        plt.imshow(det_img / 255, cmap="gray")
        plt.show()
